...
def preprocess_graph(features, adj, layer_l, layer_h, norm='sym', renorm=True):
    ...
    sm_fea_s_sum_l = torch.zeros_like(torch.FloatTensor(sp.csr_matrix(features).toarray()))
    sm_fea_s_sum_h = torch.zeros_like(sm_fea_s_sum_l)

    sm_fea_s = sp.csr_matrix(features).toarray()

    for i in range(len(reg_l)):
        adjs_l.append(ident - (reg_l[i] * laplacian))
    for a in adjs_l:
        sm_fea_s = a.dot(sm_fea_s)
        sm_fea_s_sum_l += torch.FloatTensor(sm_fea_s)

    sm_fea_s = sp.csr_matrix(features).toarray()
    for i in range(len(reg_h)):
        adjs_h.append(reg_h[i] * laplacian)
    for a in adjs_h:
        sm_fea_s = a.dot(sm_fea_s)
        sm_fea_s_sum_h += torch.FloatTensor(sm_fea_s)

    avg_fea_s_l = sm_fea_s_sum_l / (len(reg_l))
    sm_fea_s_list.append(torch.FloatTensor(avg_fea_s_l))
    avg_fea_s_h = sm_fea_s_sum_h / (len(reg_h))
    sm_fea_s_list.append(torch.FloatTensor(avg_fea_s_h))

    return sm_fea_s_list
...
def calculate_weighted_features_z(alpha, beta, sm_fea_s_list, sm_fea_s_noise):
    ...
    sm_fea_s_l = sm_fea_s_list[0].to(args.device)
    sm_fea_s_noise = sm_fea_s_noise.to(args.device)
    sm_fea_s_h = sm_fea_s_list[1].to(args.device)

    sm_fea_s_hn = alpha * sm_fea_s_h + (1 - alpha) * sm_fea_s_noise
    sm_fea_s_list_lhn = beta * sm_fea_s_l + (1 - beta) * sm_fea_s_hn

    return sm_fea_s_list_lhn

def add_gaussian_noise(X, sigma_X):
    """Add Gaussian noise to the input."""
    X = torch.tensor(X) if not isinstance(X, torch.Tensor) else X
    noise = torch.normal(mean=0, std=sigma_X, size=X.size())
    return X + noise
